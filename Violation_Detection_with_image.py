...
def helmet_and_triple_riding_detection(image_path):
    ...
    if helmet_results[0].boxes is not None:
        for box in helmet_results[0].boxes:
            ...
            if int(box.cls[0]) == 1:  
                cv2.putText(annotated_image, "Helmet Detected", (x1, y1 - 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 4)
            if int(box.cls[0]) == 2:
    ...
    #cv2.namedWindow("Helmet and Triple Riding Detection", cv2.WINDOW_NORMAL)
